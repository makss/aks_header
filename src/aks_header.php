...
function aks_header($atts)
{
    ...
    if ($gzip && !in_array('zlib output compression', ob_list_handlers())) {
        $aks_header_gzip = 1;
    }

    if ($value) {
        $aks_headers[$name] = $value;
    }
    if ($nodebug || $file) {
    ...
    if ($file && preg_match('/^.*\.(.*)$/', $thisarticle['url_title'], $mm)) {
        if ($ext[$mm[1]]) {
            $aks_headers['Content-Type'] = $ext[$mm[1]];
        }
        if ($mm[1] !='xml') {
            $aks_headers['Last-Modified'] = safe_strftime('rfc822', $thisarticle['modified']);
        }
    }
    if ($nocache) {
...
function aks_header_callback($html)
{
    ...
    if (isset($aks_headers) && is_array($aks_headers)) {
        foreach ($aks_headers as $name => $value) {
            header("$name: $value");
        }
    }
    ...
    if ($aks_header_strip) {
        ...
        foreach (array_reverse($aks_header_block, true) as $key => $block) {
    ...
    if ($aks_header_gzip
        ...
        if ($version < 6 || ($version == 6 && !strstr($_SERVER['HTTP_USER_AGENT'], 'SV1'))) {
            $aks_header_gzip=0;
        }
    }
    ...
    if ($aks_header_etag && $production_status == 'live') {
        ...
        if ($match = serverSet('HTTP_IF_NONE_MATCH')) {
            // TODO:   If-None-Match "*"
            if (in_array($etag, do_list($match))) {
